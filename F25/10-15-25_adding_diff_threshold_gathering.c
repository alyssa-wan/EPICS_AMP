...
int relaxed = 0;
int flexed = -1;
long avgRelaxed[] = {0,0,0,0,0};
long avgFlexed[] = {0,0,0,0,0};


long initialThreshold[] = {0,0,0,0,0};
const int thresholdWindowSize = windowSize * 6;
long diffThreshold[] = {0, 0, 0, 0, 0};
...
void readSensor() {
  if (start != -1) {
    ...
    if (flexed > thresholdWindowSize ) {
      ...
      diffThreshold[i] = avgFlexed[i] - initialThreshold[i];
      
      Serial.print("sum relaxed = ");
      Serial.print(sumArrRelaxed[i]);
      Serial.print("\n");
      Serial.print("avg relaxed = ");
      Serial.print(avgRelaxed[i]);
      Serial.print("\n");
      Serial.print("sum flex = ");
      Serial.print(sumArrFlexed[i]);
      Serial.print("\n");
      Serial.print("avg flex = ");
      Serial.print(avgFlexed[i]);
      Serial.print("\n");
      Serial.print("initial threshold = ");
      Serial.print(initialThreshold[i]);
      Serial.print("\n");
      Serial.print("diff thresh = ");
      Serial.print(diffThreshold[i]);
      Serial.print("\n");
  ...
  if (windowIndex == (windowSize - 1)) {
    for (int i = 0; i < num_fingers; i++) { // 1 is usually num_pins
      ...
      if ((abs(prevTriggerArr[i] - triggerArr[i]) > diffThreshold[i]) && (just_triggered[i] <= 0)) {
      //if ((just_triggered <= 0)) {
